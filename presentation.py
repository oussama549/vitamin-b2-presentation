...
# ============================================================
    ...
    with col1:
        ...
        <div class="glass-card">
            <h3>⚡ Principle</h3>
            <p>Riboflavin absorbs at <strong style="color:#00ff88;">λ = 444 nm</strong> (secondary at 267 nm). Absorbance ∝ concentration (Beer–Lambert).</p>
        </div>
        """, unsafe_allow_html=True)
        st.markdown("""
        <div class="glass-card">
            <h3>🔬 Procedure</h3>
            <ul style="color:#c8d6e5; padding-left:1.2rem;">
                <li>Prepare standards (0–10 µg/mL)</li>
                <li>Measure absorbance at 444 nm</li>
                <li>Construct calibration curve</li>
                <li>Quantify API sample</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        st.markdown("""
        <div class="highlight">
            <strong>✅ Outcome:</strong> API purity confirmed if absorbance falls within linear range.
        </div>
        """, unsafe_allow_html=True)
    
    with col2:
        st.markdown('<p style="color:#8899bb; font-size:0.9rem;">Calibration curve · Riboflavin @ 444 nm</p>', unsafe_allow_html=True)
        conc = st.slider("Concentration (µg/mL)", 0.0, 10.0, 5.0, 0.5)
        absorbance = 0.0485 * conc + 0.001
        
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=[0, 2, 4, 6, 8, 10],
            y=[0.000, 0.098, 0.195, 0.292, 0.388, 0.485],
            mode='markers+lines',
        ...
        with col_a:
            ...
            <div class="metric-box">
                <div class="value">{absorbance:.3f}</div>
